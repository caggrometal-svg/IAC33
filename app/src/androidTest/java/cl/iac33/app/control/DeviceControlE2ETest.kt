    ...
    fun setUp() {
        ...
        server.dispatcher = object : Dispatcher() {
            override fun dispatch(request: RecordedRequest): MockResponse {
                return when {
                    request.path == "/v1/devices/enroll" -> MockResponse()
                        .setResponseCode(201)
                        .setBody("{\"ok\":true}")
                    request.path == "/v1/device/commands/claim-next" -> MockResponse()
                        .setResponseCode(200)
                        .setBody("{\"command\":{\"id\":\"e2e-command\",\"type\":\"PING\",\"payload\":{},\"idempotency_key\":\"e2e-key\",\"expires_at\":\"2099-01-01T00:00:00Z\",\"status\":\"CLAIMED\"}}")
                    request.path == "/v1/device/commands/e2e-command/execute" -> MockResponse()
                        .setResponseCode(200)
                        .setBody("{\"command\":{\"id\":\"e2e-command\",\"status\":\"EXECUTING\"}}")
                    request.path == "/v1/device/commands/e2e-command/succeed" -> MockResponse()
                        .setResponseCode(200)
                        .setBody("{\"command\":{\"id\":\"e2e-command\",\"status\":\"SUCCEEDED\"}}")
