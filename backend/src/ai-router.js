...
const PROVIDER_HEALTH_URLS = {
  ...
  andrew2: 'https://andrew2-api.onrender.com/health',
  ollama: null,
...
function healthConfigured(id) {
  const provider = providers[id];
  if (!provider) return false;
  if (id === 'ollama') return Boolean(process.env.IAC33_OLLAMA_ENDPOINT);
...
function healthUrl(id) {
  if (id === 'ollama') {
    const endpoint = String(process.env.IAC33_OLLAMA_ENDPOINT || '').trim().replace(/\/$/, '');
    return endpoint ? (endpoint.endsWith('/v1/chat/completions') ? endpoint.replace('/v1/chat/completions', '/api/tags') : endpoint + '/api/tags') : null;
  }
  if (id === 'cloudflare') {
  ...
  const override = process.env['AI_HEALTHCHECK_URL_' + id.toUpperCase()];
  return override || PROVIDER_HEALTH_URLS[id] || null;
}

async function probeConnectivity(url, headers = {}, timeoutMs = CONNECTIVITY_TIMEOUT_MS) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    ...
    if (error?.name === 'AbortError') return { status: 504, ok: false, timeout: true };
    return { status: null, ok: false, error: String(error?.message || error).slice(0, 160) };
  } finally {
    clearTimeout(timer);
  }
}
...
export async function checkProviderHealth(id, { force = false } = {}) {
  ...
  if (!healthConfigured(id)) return { provider: id, state: 'NOT_CONFIGURED' };
  const now = Date.now();
  const cached = connectivityHealthCache.get(id);
  if (!force && cached && now - cached.checkedAt < CONNECTIVITY_CACHE_MS) return { ...cached.result, cached: true };
  let url = healthUrl(id);
  if (!url) return { provider: id, state: 'NOT_VERIFIABLE' };
  const headers = { accept: 'application/json' };
  const provider = providers[id];
  if (id === 'gemini') url += (url.includes('?') ? '&' : '?') + 'key=' + encodeURIComponent(process.env.GEMINI_API_KEY);
  else if (id === 'anthropic') {
    headers['x-api-key'] = process.env.ANTHROPIC_API_KEY;
    headers['anthropic-version'] = '2023-06-01';
  } else if (id === 'horde') {
    headers['Client-Agent'] = 'IAC33:2.0';
  } else if (id === 'andrew2' && process.env.IAC33_ANDREW2_API_KEY) {
    headers.authorization = 'Bearer ' + process.env.IAC33_ANDREW2_API_KEY;
  } else if (provider.key && process.env[provider.key]) {
    headers.authorization = 'Bearer ' + process.env[provider.key];
  }
  const started = Date.now();
  const observed = await probeConnectivity(url, headers);
  const state = observed.ok ? 'OK'
    ...
    : observed.error ? 'NETWORK_ERROR' : 'ERROR';
  const result = {
    provider: id,
    state,
...
export async function checkInternetHealth() {
  const started = Date.now();
  const observed = await probeConnectivity(process.env.AI_INTERNET_HEALTHCHECK_URL || 'https://www.google.com/generate_204');
  return {
    state: observed.ok || observed.status === 204 ? 'OK' : observed.timeout ? 'TIMEOUT' : 'ERROR',
    status: observed.status,
    latencyMs: Date.now() - started,
    ...(observed.error ? { error: observed.error } : {})
  };
...
export async function diagnoseConnectivity() {
  const order = parseList(process.env.AI_PROVIDER_ORDER, FREE_PROVIDER_DEFAULTS.join(','));
  const unique = [...new Set(order)];
  const internet = await checkInternetHealth();
  const providersHealth = await Promise.all(unique.map((id) => checkProviderHealth(id, { force: true })));
  const circuits = unique.map((id) => ({ provider: id, open: circuitOpen(id) }));
  return { internet, backendRouter: { state: 'OK' }, androidToBackend: { state: 'NOT_VERIFIED' }, providers: providersHealth, circuits, order: unique, generatedAt: new Date().toISOString() };
}

export function isConnectivityDiagnosticRequest(messages) {
  const userText = messages.filter((m) => m?.role === 'user').map((m) => String(m.content || '')).join('\n').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  return userText.includes('diagnostico_conectividad') || /diagnostico(?:\s+completo)?\s+de\s+conectividad/.test(userText);
}

export function formatConnectivityDiagnostics(diagnostic) {
  const lines = [
    'INTERNET: ' + diagnostic.internet.state + (diagnostic.internet.latencyMs != null ? ' · ' + diagnostic.internet.latencyMs + ' ms' : ''),
    'BACKEND: OK',
    'ROUTER: OK',
    'ANDROID→BACKEND: NO VERIFICADO'
  ];
  for (const p of diagnostic.providers) {
    lines.push(p.provider.toUpperCase() + ': ' + p.state
      + (p.status ? ' · HTTP ' + p.status : '')
      + (p.latencyMs != null ? ' · ' + p.latencyMs + ' ms' : '')
      + (p.error ? ' · ' + p.error : ''));
  }
  const open = diagnostic.circuits.filter((x) => x.open).map((x) => x.provider);
  lines.push('RUTA ORDEN: ' + (diagnostic.order.join(' → ') || 'NINGUNA'));
  lines.push('CIRCUITOS ABIERTOS: ' + (open.length ? open.join(', ') : 'NINGUNO'));
  const firstOk = diagnostic.providers.find((p) => p.state === 'OK');
  lines.push(firstOk
    ? 'RUTA DISPONIBLE: ' + firstOk.provider
    : diagnostic.internet.state !== 'OK'
      ? 'PUNTO DE FALLA: salida de Internet del backend o URL de prueba inaccesible'
      : 'PUNTO DE FALLA: ninguna ruta IA remota verificable/operativa');
  lines.push('ARCHIVO/MÓDULO: backend/src/ai-router.js');
  lines.push('PRUEBA POSTERIOR: repetir diagnostico_conectividad y luego enviar una pregunta normal');
  return lines.join('\n');
}
...
export async function generateWithFreePool({ messages, timeoutMs = 18000, signal }) {
  if (isConnectivityDiagnosticRequest(messages)) {
    const diagnostic = await diagnoseConnectivity();
    return { text: formatConnectivityDiagnostics(diagnostic), model: 'connectivity-diagnostics', provider: 'diagnostics', diagnostics: diagnostic.providers };
  }
  const order = [...new Set(
